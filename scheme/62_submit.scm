...
(define (fold f init lis1 lis2)
  (if (or (null? lis1) (null? lis2))
      init
      (fold f (f init (car lis1) (car lis2))
               (cdr lis1)
               (cdr lis2))))
...
(define (base-apply env fun args)
  (cond ((data-closure? fun)
         (if (= (length (closure-params fun)) (length args))
             (let* ((env-new (fold define-var (extend-env (closure-env fun)) (closure-params fun) args))) 
               (cons env (cdr (base-eval env-new (closure-body fun)))))
